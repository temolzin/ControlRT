...
public class SocketServidorObservacionesAsesorAcaConsultarAlumno implements Runnable {
	...
	public SocketServidorObservacionesAsesorAcaConsultarAlumno() {
		try {
			this.servidor = new ServerSocket(5014, 50);
			this.conex = ConexionPost.getInstance();
		} catch (IOException | ClassNotFoundException e) {
			Main.vistaServer.setTextArea("Error en SocketServidorObservacionesAcademico: " + e.getMessage() + "\n");
		}
		this.cliente = null;
	...
	private void consultarAlumno() {
		try {
			...
			this.cadena = "select * from alumno where matricula = " + this.matricula + " and id_asesorAca = " + Login.getMatricula();
			...
			Main.vistaServer
					.setTextArea("Error en SocketServidorObservacionesAcademico metodo consultarAlumno: " + e.getMessage() + "\n");
		}
	}
}
