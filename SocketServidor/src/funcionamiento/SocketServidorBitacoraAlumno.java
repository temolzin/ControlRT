...
public class SocketServidorBitacoraAlumno implements Runnable {
	...
	public SocketServidorBitacoraAlumno() {
	...
	public void run() {
		try {
			...
			this.servidor = new ServerSocket(5020, 50);
			...
			Main.vistaServer.setTextArea("Error en SocketServidorBitacoraAlumno ClassNot: " + e.getMessage() + "\n");
		} catch (IOException e) {
			Main.vistaServer.setTextArea("Error en SocketServidorBitacoraAlumno IOException: " + e.getMessage() + "\n");
		}
	}

	private void consultarBitacora() {
		try {
			this.avancesAlumno = conex.consultar("select * from avancesalumno where matriculaalumno = " + Login.getMatricula());
			this.evaluacionAcademico = conex.consultar("select * from evaluacionaca where matriculaalumno = " + Login.getMatricula());
			this.evaluacionIndustrial = conex.consultar("select * from evaluacionind where matriculaalumno = " + Login.getMatricula());
			this.observacionAcademico = conex.consultar("select * from observacionesaca where matriculaalumno = " + Login.getMatricula());
			this.observacionIndustrial = conex.consultar("select * from observacionesind where matriculaalumno = " + Login.getMatricula());
			...
			Main.vistaServer.setTextArea("Error en SocketServidorBitacoraAlumno SQLException: " + e.getMessage() + "\n");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
